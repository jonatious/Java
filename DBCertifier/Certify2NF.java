 ...
 *
 */
public class Certify2NF {
	
    public static boolean Certify2NF(String table, String[] table_data){
        MainProject.error = "";
        int x=0,y=0;
        String[][] new_tables = new String[50][50];
        ArrayList Keys = DataAccessLayer.canKeyOnly(table_data);
        ArrayList nonKeys = DataAccessLayer.nonKeyOnly(table_data);
        ArrayList subsetsKeys = DataAccessLayer.subsetOfKey(Keys);
        ArrayList subTableColumns = new ArrayList();
        ArrayList nonTableColumns = new ArrayList();

        for(int j=0;j<subsetsKeys.size();j++){
            String LHS = subsetsKeys.get(j).toString();
        ...
        if(count==0){
            System.out.println("the table '" + table + "' is in 2NF.");
            return true;
        }
        else{
            System.out.println("the table '" + table + "' is not in 2NF.");
		...
		Vertica_Connection.Create_Query(Drop_Query);
            ...
            for(int t=0;t< table_data.length;t++){
                ...
                if(!nonTableColumns.contains(temp_column))
                    columns += temp_column + ",";
            }
